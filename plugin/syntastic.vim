...
let g:SYNTASTIC_VERSION = '3.5.0-72'
...
" debug constants
let     g:SYNTASTIC_DEBUG_TRACE         = 1
lockvar g:SYNTASTIC_DEBUG_TRACE
let     g:SYNTASTIC_DEBUG_LOCLIST       = 2
lockvar g:SYNTASTIC_DEBUG_LOCLIST
let     g:SYNTASTIC_DEBUG_NOTIFICATIONS = 4
lockvar g:SYNTASTIC_DEBUG_NOTIFICATIONS
let     g:SYNTASTIC_DEBUG_AUTOCOMMANDS  = 8
lockvar g:SYNTASTIC_DEBUG_AUTOCOMMANDS
let     g:SYNTASTIC_DEBUG_VARIABLES     = 16
lockvar g:SYNTASTIC_DEBUG_VARIABLES
let     g:SYNTASTIC_DEBUG_CHECKERS      = 32
lockvar g:SYNTASTIC_DEBUG_CHECKERS
...
function! s:BufReadPostHook() " {{{2
    if g:syntastic_check_on_open
        call syntastic#log#debug(g:SYNTASTIC_DEBUG_AUTOCOMMANDS,
...
function! s:UpdateErrors(auto_invoked, ...) " {{{2
    call syntastic#log#debugShowVariables(g:SYNTASTIC_DEBUG_TRACE, 'version')
    call syntastic#log#debugShowOptions(g:SYNTASTIC_DEBUG_TRACE, s:DEBUG_DUMP_OPTIONS)
    call syntastic#log#debugDump(g:SYNTASTIC_DEBUG_VARIABLES)
    call syntastic#log#debug(g:SYNTASTIC_DEBUG_TRACE, 'UpdateErrors' . (a:auto_invoked ? ' (auto)' : '') .
    ...
    if syntastic#util#var('always_populate_loc_list') || do_jump
        call syntastic#log#debug(g:SYNTASTIC_DEBUG_NOTIFICATIONS, 'loclist: setloclist (new)')
        call setloclist(0, loclist.getRaw())
        let w:syntastic_loclist_set = 1
        if run_checks && do_jump && !loclist.isEmpty()
            call syntastic#log#debug(g:SYNTASTIC_DEBUG_NOTIFICATIONS, 'loclist: jump')
            silent! lrewind
...
function! s:CacheErrors(checker_names) " {{{2
    call syntastic#log#debug(g:SYNTASTIC_DEBUG_TRACE, 'CacheErrors: ' .
    ...
    if !s:skipFile()
        " debug logging {{{3
        call syntastic#log#debugShowVariables(g:SYNTASTIC_DEBUG_TRACE, 'aggregate_errors')
        call syntastic#log#debug(g:SYNTASTIC_DEBUG_TRACE, 'getcwd() = ' . getcwd())
        " }}}3
        ...
        for checker in clist
            ...
            if !checker.isAvailable()
                call syntastic#log#debug(g:SYNTASTIC_DEBUG_TRACE, 'CacheErrors: Checker ' . cname . ' is not available')
                let unavailable_checkers += 1
                continue
            endif

            call syntastic#log#debug(g:SYNTASTIC_DEBUG_TRACE, 'CacheErrors: Invoking checker: ' . cname)
            ...
            if !loclist.isEmpty()
                ...
                if checker.getWantSort() && !sort_aggregated_errors
                    call loclist.sort()
                    call syntastic#log#debug(g:SYNTASTIC_DEBUG_LOCLIST, 'sorted:', loclist)
                endif
        ...
        if len(clist) == unavailable_checkers
            if !empty(a:checker_names)
                ...
                call syntastic#log#debug(g:SYNTASTIC_DEBUG_TRACE, 'CacheErrors: no checkers available for ' . &filetype)
            endif
        endif
        " }}}3

        call syntastic#log#debug(g:SYNTASTIC_DEBUG_LOCLIST, 'aggregated:', newLoclist)
        if sort_aggregated_errors
            call newLoclist.sort()
            call syntastic#log#debug(g:SYNTASTIC_DEBUG_LOCLIST, 'sorted:', newLoclist)
        endif
    endif
...
function! SyntasticMake(options) " {{{2
    call syntastic#log#debug(g:SYNTASTIC_DEBUG_TRACE, 'SyntasticMake: called with options:', a:options)
    ...
    call syntastic#log#debug(g:SYNTASTIC_DEBUG_LOCLIST, 'checker output:', err_lines)

    if has_key(a:options, 'Preprocess')
        let err_lines = call(a:options['Preprocess'], [err_lines])
        call syntastic#log#debug(g:SYNTASTIC_DEBUG_LOCLIST, 'preprocess (external):', err_lines)
    elseif has_key(a:options, 'preprocess')
        let err_lines = call('syntastic#preprocess#' . a:options['preprocess'], [err_lines])
        call syntastic#log#debug(g:SYNTASTIC_DEBUG_LOCLIST, 'preprocess:', err_lines)
    endif
    lgetexpr err_lines
    ...
    call syntastic#log#debug(g:SYNTASTIC_DEBUG_LOCLIST, 'raw loclist:', errors)
    ...
    if has_key(a:options, 'Postprocess') && !empty(a:options['Postprocess'])
        ...
        call syntastic#log#debug(g:SYNTASTIC_DEBUG_LOCLIST, 'postprocess (external):', errors)
    elseif has_key(a:options, 'postprocess') && !empty(a:options['postprocess'])
        for rule in a:options['postprocess']
            let errors = call('syntastic#postprocess#' . rule, [errors])
        endfor
        call syntastic#log#debug(g:SYNTASTIC_DEBUG_LOCLIST, 'postprocess:', errors)
    endif
...
function! s:skipFile() " {{{2
    ...
    if skip
        call syntastic#log#debug(g:SYNTASTIC_DEBUG_TRACE, 'skipFile: skipping')
    endif
    return skip
endfunction " }}}2
