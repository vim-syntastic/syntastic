...
function! g:SyntasticHighlightingNotifier.refresh(loclist) " {{{2
    if self.enabled()
        call syntastic#log#debug(g:SYNTASTIC_DEBUG_NOTIFICATIONS, 'highlighting: refresh')
        call self._reset()
        let buf = bufnr('')
...
function! g:SyntasticHighlightingNotifier.reset(loclist) " {{{2
    if s:has_highlighting
        call syntastic#log#debug(g:SYNTASTIC_DEBUG_NOTIFICATIONS, 'highlighting: reset')
        call self._reset()
    endif
endfunction " }}}2
