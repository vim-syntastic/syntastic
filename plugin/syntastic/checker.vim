...
function! g:SyntasticChecker.getLocListRaw() " {{{2
    ...
    try
        ...
        call syntastic#log#debug(g:SYNTASTIC_DEBUG_TRACE, 'getLocList: checker ' . name . ' returned ' . v:shell_error)
    catch /\m\C^Syntastic: checker error$/
        let list = []
        call syntastic#log#error('checker ' . name . ' returned abnormal status ' . v:shell_error)
    endtry
    call self._populateHighlightRegexes(list)
    call syntastic#log#debug(g:SYNTASTIC_DEBUG_LOCLIST, name . ' raw:', list)
...
function! g:SyntasticChecker.log(msg, ...) " {{{2
    ...
    if a:0 > 0
        call syntastic#log#debug(g:SYNTASTIC_DEBUG_CHECKERS, leader . a:msg, a:1)
    else
        call syntastic#log#debug(g:SYNTASTIC_DEBUG_CHECKERS, leader . a:msg)
    endif
endfunction " }}}2
...
function! g:SyntasticChecker._quietMessages(errors) " {{{2
    ...
    call syntastic#log#debug(g:SYNTASTIC_DEBUG_LOCLIST, 'quiet_messages filter:', quiet_filters)

    if !empty(quiet_filters)
        call syntastic#util#dictFilter(a:errors, quiet_filters)
        call syntastic#log#debug(g:SYNTASTIC_DEBUG_LOCLIST, 'filtered by quiet_messages:', a:errors)
    endif
endfunction " }}}2
