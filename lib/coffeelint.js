...
(function() {
  ...
  LineLinter = (function() {
    ...
    LineLinter.prototype.checkComments = function() {
      var cmd, r, rules;
      r = regexes.configStatement.exec(this.line);
      cmd = r != null ? r[1] : void 0;
      rules = r != null ? r.slice(2) : void 0;
      if (rules) {
  ...
  coffeelint.lint = function(source, userConfig) {
    var all_errors, astErrors, config, disabled, disabled_initially, error, errors, lconfig, lexErrors, lexicalLinter, lineErrors, lineLinter, r, rule, tmp, tokensByLine, _i, _j, _k, _len, _len1, _len2, _ref, _ref1;
    if (userConfig == null) {
      userConfig = {};
    }
    ...
    lconfig = config;
    disabled_initially = [];
    _ref = block_config['enable'];
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      rule = _ref[_i];
      if (!(rule in config)) {
        disabled.push(rule);
        lconfig[rule] = defaults({
          level: 'warn'
        });
      }
    }
    ...
    for (_j = 0, _len1 = all_errors.length; _j < _len1; _j++) {
      error = all_errors[_j];
      if ((error.lineNumber - 1) in block_config['disable']) {
        disabled.concat(block_config['disable'][error.lineNumber - 1]);
      }
      if ((error.lineNumber - 1) in block_config['enable']) {
        if (block_config['enable'][error.lineNumber - 1].length === 0) {
          disabled = disabled_initially;
        } else {
          tmp = [];
          for (_k = 0, _len2 = disabled.length; _k < _len2; _k++) {
            r = disabled[_k];
            if (__indexOf.call(block_config['enable'][error.lineNumber - 1], r) < 0) {
              tmp.push(r);
            }
          }
          disabled = r;
      ...
      if (_ref1 = error.rule, __indexOf.call(disabled, _ref1) < 0) {
        errors.push(error);
      }
    }
    return errors;
