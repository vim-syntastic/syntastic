...
(function() {
  var ASTLinter, CoffeeScript, ERROR, IGNORE, LexicalLinter, LineLinter, RULES, WARN, coffeelint, createError, defaults, extend, mergeDefaultConfig, regexes, suppressions,
    __slice = [].slice,
  ...
  LineLinter = (function() {
    ...
    LineLinter.prototype.checkComments = function() {
      var rules, _ref, _ref1;
      rules = (_ref = /#!SUPPRESS(.*)/.exec(this.line)) != null ? (_ref1 = _ref[1]) != null ? _ref1.trim().split(' ') : void 0 : void 0;
      if (rules) {
        suppressions[this.lineNumber] = rules;
      }
      return null;
    };
  ...
  coffeelint.lint = function(source, userConfig) {
    var all_errors, astErrors, config, error, errors, lexErrors, lexicalLinter, lineErrors, lineLinter, tokensByLine, _i, _len, _ref;
    if (userConfig == null) {
      userConfig = {};
    }
    ...
    for (_i = 0, _len = all_errors.length; _i < _len; _i++) {
      error = all_errors[_i];
      if ((error.lineNumber - 1) in suppressions) {
        if (_ref = error.rule, __indexOf.call(suppressions[error.lineNumber - 1], _ref) < 0) {
          errors.push(error);
        }
      } else {
        errors.push(error);
